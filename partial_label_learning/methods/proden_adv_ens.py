""" Module for PRODEN ensemble with adversarial training. """
...
def _train_model(
    ...
    for _ in range(200):
        for idx, inputs_i, partial_targets_i, w_ij in data_loader:
            # Move to device
            inputs_i: torch.Tensor = inputs_i.to(device).requires_grad_()
            partial_targets_i = partial_targets_i.to(device)
            w_ij = w_ij.to(device)

            # Forward-backward pass
            probs: torch.Tensor = model(inputs_i)
            loss = torch.mean(torch.sum(
                w_ij * -torch.log(probs + 1e-10), dim=1))

            # Backprop and generate adversarial examples; no optimization step
            optimizer.zero_grad()
            loss.backward(retain_graph=True)
            assert inputs_i.grad is not None
            inputs_adversarial = inputs_i + 0.01 * torch.sign(inputs_i.grad)

            # Loss with adversarial examples
            optimizer.zero_grad()
            probs_adv: torch.Tensor = model(inputs_adversarial)
            loss_adv = loss + torch.mean(torch.sum(
                w_ij * -torch.log(probs_adv + 1e-10), dim=1))
            loss_adv.backward()
            optimizer.step()
...
class ProdenAdvEns(PllBaseClassifier):
    """
    PRODEN by Lv et al.,
